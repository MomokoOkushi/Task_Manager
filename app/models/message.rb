class Message < ApplicationRecord
  ...
  def self.received_latest_messages(current_user)
    ...
    send_user_ids.map{|send_user_id|latest_message(send_user_id) }.sort_by{|message| message.created_at}.reverse
  end

  def self.latest_message(send_user_id) #received_latest_messagesメソッドにて使用
    where(send_user_id: send_user_id).last
  end
end
